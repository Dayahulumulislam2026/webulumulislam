...
@section('title', $institution->name ?? 'Dayah Terpadu Ulumul Islam')
@section('description', 'Pondok Pesantren Dayah Terpadu Ulumul Islam - Pengkaderan Ulama Amilin dan Pemimpin Ummat.')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <div class="lg:col-span-7 space-y-5 scroll-reveal">
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 border border-white/15 backdrop-blur-md">
                    ...
                    <span class="text-xs font-black uppercase tracking-wider text-accent-gold">Pondok Pesantren Dayah Terpadu</span>
                </div>
                ...
                <p class="text-sm sm:text-base text-primary-100/90 leading-relaxed font-medium max-w-2xl">
                    {{ $institution->short_description ?? 'Pusat Pengkaderan Ulama Amilin, Pembinaan Akhlakul Karimah, dan Pendalaman Kitab Kuning serta Bahasa Internasional.' }}
                </p>
                ...
                <div class="relative bg-white/10 backdrop-blur-md rounded-3xl p-6 sm:p-8 border border-white/20 shadow-2xl space-y-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <span class="text-xs font-bold text-accent-gold uppercase tracking-wider">Pondok Pesantren Terpadu</span>
                        </div>
                    </div>
                    <p class="text-xs text-primary-100/80 leading-relaxed">
                        Pembinaan terpadu 24 jam dengan integrasi kurikulum kitab kuning salafiyah, tahfidz Al-Quran 30 juz, serta percakapan aktif bahasa Arab & Inggris.
                    </p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            <div class="lg:col-span-5 space-y-4 scroll-reveal">
                <span class="text-xs font-black uppercase tracking-widest text-accent-gold">Tradisi Keilmuan</span>
                <h2 class="text-2xl sm:text-3xl font-black text-primary-950">Pondok Pesantren Dayah Terpadu</h2>
                <p class="text-xs sm:text-sm text-text-sub font-medium leading-relaxed whitespace-pre-line">
                    {{ $institution->description ?? 'Dayah Terpadu Ulumul Islam mendidik santri secara intensif 24 jam dengan integrasi kurikulum kitab turots (kuning), tahfidz Al-Quran, penguasaan muhadatsah bahasa Arab dan conversation bahasa Inggris.' }}
                </p>
            </div>
            ...
            <div class="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6 scroll-reveal">
                <div class="bg-primary-50 p-6 rounded-3xl border border-primary-100 shadow-xs space-y-3">
                    <div class="w-10 h-10 rounded-xl bg-cyan-100 text-cyan-800 flex items-center justify-center font-black text-xs">
                        VISI
                    </div>
                    <h3 class="text-base font-black text-slate-900">Visi Dayah</h3>
                    <p class="text-xs text-text-sub font-medium leading-relaxed">
                        {{ $institution->vision ?? 'Melahirkan ulama amilin yang kokoh dalam aqidah, mendalam dalam ilmu fiqh, dan tanggap terhadap perkembangan zaman.' }}
                    </p>
                </div>
                ...
                <div class="bg-amber-50 p-6 rounded-3xl border border-amber-100 shadow-xs space-y-3">
                    ...
                    <h3 class="text-base font-black text-slate-900">Misi Dayah</h3>
                    <div class="text-xs text-text-sub font-medium leading-relaxed">
                        ...
                        @else
                            {{ $institution->mission_text ?? "1. Menanamkan kebiasaan ibadah.\n2. Mengkaji kitab muktabarah.\n3. Membiasakan bahasa Arab & Inggris." }}
                        @endif
                    </div>
                </div>
            </div>
...
<!-- 2. Struktur Utama Dayah & Tombol Bagan Lengkap -->
<section class="py-16 bg-bg-warm border-b border-border-main">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 scroll-reveal">
            <div>
                <span class="text-xs font-black uppercase tracking-widest text-accent-gold">Amanah & Pengasuhan</span>
                <h2 class="text-2xl sm:text-3xl font-black text-primary-950">Pimpinan & Pengasuh Dayah</h2>
                <p class="text-xs text-text-sub font-medium mt-1">Mudir Ma'had dan jajaran pimpinan pengasuhan santri.</p>
            </div>
            <a href="{{ route('structure.public', ['unit' => 'dayah']) }}" class="px-5 py-2.5 rounded-2xl bg-white hover:bg-primary-900 text-primary-900 hover:text-white border border-primary-900/20 font-extrabold text-xs uppercase tracking-wider shadow-xs transition-all inline-flex items-center gap-2 shrink-0">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 scroll-reveal">
            @foreach($topStructureMembers as $item)
                ...
                <div class="bg-white rounded-3xl p-6 border border-border-main text-center space-y-4 shadow-xs hover:shadow-lg transition-all flex flex-col justify-between">
                    <div class="space-y-3">
                        <div class="w-24 h-24 rounded-full mx-auto overflow-hidden bg-cyan-900/10 ring-4 ring-cyan-100 shadow-xs">
                            @if($member->photo_path)
                            <img src="{{ $member->photo_path }}" alt="{{ $member->name }}" class="w-full h-full object-cover">
                            @else
                            <div class="w-full h-full flex items-center justify-center bg-cyan-100 text-cyan-900 font-black text-xl">
        ...
        <div class="text-center py-12 bg-white rounded-3xl border border-dashed border-slate-300 p-8 space-y-2">
            ...
            <p class="text-xs text-slate-500">Susunan pengurus dan pengasuh Dayah sedang dalam proses pembaruan.</p>
        </div>
        @endif
    </div>
</section>

<!-- 3. List Alumni Dayah Pilihan (Maks 13, 5 Tampil Utama, Selebihnya Digeser) -->
<section class="py-16 bg-white border-b border-border-main">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 scroll-reveal">
            <div class="space-y-1">
                <span class="text-xs font-black uppercase tracking-widest text-accent-gold">Jejak Alumni Pesantren</span>
                <h2 class="text-2xl sm:text-3xl font-black text-primary-950">Sosok Alumni Dayah Pilihan</h2>
                <p class="text-xs sm:text-sm text-text-sub font-medium">Klik profil alumni untuk melihat kiprah dakwah, profesi, dan kesan para lulusan.</p>
            </div>
            @if(isset($alumni) && $alumni->count() > 5)
            <div class="flex items-center gap-2 shrink-0">
                <button type="button" onclick="scrollAlumniTrack('dayah-alumni-track', -1)" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-primary-900 hover:text-white text-slate-700 border border-slate-200 transition-colors shadow-xs flex items-center justify-center" title="Geser ke Kiri">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </button>
                <button type="button" onclick="scrollAlumniTrack('dayah-alumni-track', 1)" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-primary-900 hover:text-white text-slate-700 border border-slate-200 transition-colors shadow-xs flex items-center justify-center" title="Geser ke Kanan">
        ...
        <div id="dayah-alumni-track" class="flex gap-4 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-accent-gold/40 snap-x scroll-smooth">
            ...
            <div onclick="openAlumniDetailModal({{ json_encode($alum) }})" class="w-[85%] sm:w-[calc(50%-12px)] md:w-[calc(33.333%-14px)] lg:w-[calc(20%-13px)] shrink-0 bg-bg-warm rounded-3xl p-5 border border-border-main shadow-xs hover:shadow-xl hover:border-primary-900/40 transition-all text-center flex flex-col justify-between cursor-pointer group hover:-translate-y-1 snap-start">
                <div class="space-y-3">
                    <div class="relative w-20 h-20 mx-auto">
                        @if($alum->photo_path)
                        <img src="{{ $alum->photo_path }}" alt="{{ $alum->name }}" class="w-20 h-20 rounded-2xl object-cover mx-auto ring-2 ring-cyan-800 shadow-xs">
                        @else
                        <div class="w-20 h-20 rounded-2xl mx-auto bg-cyan-100 text-cyan-900 flex items-center justify-center font-black text-lg border border-cyan-200">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-cyan-800 font-bold uppercase tracking-wider mt-0.5 line-clamp-1">{{ $alum->position_or_program ?? $alum->career_type ?? 'Alumni' }}</p>
        ...
        <div class="text-center py-12 bg-bg-warm rounded-3xl border border-dashed border-slate-300 p-8 space-y-2">
            <p class="font-bold text-slate-700 text-sm">Belum Ada Profil Alumni Dayah</p>
            <p class="text-xs text-slate-500">Data alumni santri Dayah sedang dalam tahap kurasi dan penginputan.</p>
        </div>
        @endif
    </div>
</section>

<!-- 4. 5 Berita Terbaru Dayah (Kotak Persegi Panjang Vertikal) -->
<section class="py-16 bg-bg-warm border-b border-border-main">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 scroll-reveal">
            <div>
                <span class="text-xs font-black uppercase tracking-widest text-accent-gold">WARTA & KABAR PESANTREN</span>
                <h2 class="text-2xl sm:text-3xl font-black text-primary-950">Berita & Informasi Dayah</h2>
                <p class="text-xs text-text-sub font-medium mt-1">Publikasi warta, pengajian, dan kegiatan pesantren.</p>
            </div>
            <a href="{{ route('news.index', ['unit' => 'dayah']) }}" class="px-5 py-2.5 rounded-2xl bg-white hover:bg-primary-900 text-primary-900 hover:text-white border border-primary-900/20 font-extrabold text-xs uppercase tracking-wider shadow-xs transition-all inline-flex items-center gap-2 shrink-0">
        ...
        @else
        <div class="text-center py-12 bg-white rounded-3xl border border-dashed border-slate-300 p-8 space-y-2">
            <p class="font-bold text-slate-700 text-sm">Belum Ada Berita Dayah</p>
            <p class="text-xs text-slate-500">Saat ini belum ada artikel berita atau warta yang dipublikasikan pada kategori Dayah.</p>
        </div>
        @endif
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 scroll-reveal">
            <div>
                ...
                <h2 class="text-2xl sm:text-3xl font-black text-primary-950">Galeri Foto Dayah</h2>
                <p class="text-xs text-text-sub font-medium mt-1">Dokumentasi aktivitas ibadah, pengajian, dan kehidupan santri.</p>
            </div>
            <a href="{{ route('gallery.index', ['unit' => 'dayah']) }}" class="px-5 py-2.5 rounded-2xl bg-bg-warm hover:bg-primary-900 text-primary-900 hover:text-white border border-primary-900/20 font-extrabold text-xs uppercase tracking-wider shadow-xs transition-all inline-flex items-center gap-2 shrink-0">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 scroll-reveal">
            ...
            <div class="group relative rounded-2xl overflow-hidden aspect-square bg-slate-100 border border-slate-200 shadow-xs hover:shadow-xl transition-all">
                <img src="{{ $photo->file_path }}" alt="{{ $photo->title ?: 'Dokumentasi Dayah' }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-3 flex flex-col justify-end">
                    <p class="text-[11px] font-bold text-white leading-tight line-clamp-2">{{ $photo->title ?: ($photo->album->title ?? 'Galeri Dayah') }}</p>
                </div>
            </div>
            @endforeach
        </div>
        @else
        <div class="text-center py-12 bg-bg-warm rounded-3xl border border-dashed border-slate-300 p-8 space-y-2">
            <p class="font-bold text-slate-700 text-sm">Belum Ada Foto Dokumentasi Dayah</p>
            <p class="text-xs text-slate-500">Dokumentasi foto kegiatan Dayah akan ditampilkan di sini.</p>
        </div>
        @endif
    </div>
</section>
